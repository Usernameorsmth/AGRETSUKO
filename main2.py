...
class Restaurant0rderManagement:
    def __init__(self, root):
     ...
     for i, (item, price) in enumerate(self.meanu_items.items(), start=1):
     ...
     self.currency_var.tracel('w',self.update_menu_prices)
     order_button=ttk.Button(frame,
                             text="Place Order",
                             commamnd=self.place_order)
    ...
    def setup_background(self,root):
            ...
            canvas.pack()
            original_image = tk.PhotoImage(file="background.jpg")
    ...
    def place_order(self):
            ...
            currency = self.currency_var.ger()
            symbol="?" if currency == "INR" else "$"
            rate = self.exchange_rate if currency == "INR" else 1
            ...
            if total_cost > 0:
                    ...
                    messagebox.showerror("Error", "Please order at leasr one time.")
if __name__=="__main__":
        root = tk.Tk()
        app= Restaurant0rderManagement(root)
